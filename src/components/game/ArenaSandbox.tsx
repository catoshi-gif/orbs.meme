  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      function addPedestal(x:number,z:number,kind:ArenaPowerKind){
        ...
        const badge=makeLabel(THREE,kind==="superjump"?"↑ DOUBLE JUMP":kind==="blaster"?"● BLASTER":"» SUPER SPEED",accent);badge.position.set(x,4.35,z);badge.scale.multiplyScalar(.82);scene.add(badge);
      ...
      function addPickup(rx:number,rz:number,surfaceY:number,kind:PickupKind){
        ...
        const badge=makeLabel(THREE,badgeText,color);badge.position.set(0,2.35,0);badge.scale.multiplyScalar(.72);group.add(badge);
      ...
      function updateProjectiles(now:number,dt:number){for(const o of orbs){if(o.alive&&o.blasterActive&&o.powerKind==="blaster"&&now<o.powerExpiresAt&&now>=o.nextShotAt){fireBullet(o,now);o.nextShotAt=now+145}}for(let i=projectiles.length-1;i>=0;i--){const b=projectiles[i]!,age=now-b.born;if(age>b.life){scene.remove(b.mesh);b.mesh.geometry.dispose();(b.mesh.material as import("three").Material).dispose();projectiles.splice(i,1);continue}b.position.addScaledVector(b.velocity,dt);b.mesh.position.copy(b.position);let hit=false;for(const t of orbs){if(!t.alive||t===b.owner)continue;const p=t.body.translation();if(Math.hypot(p.x-b.position.x,p.y-b.position.y,p.z-b.position.z)>PHYSICS.ballRadius+.22*s)continue;const invulnerable=now<t.speedUntil,damage=invulnerable?0:8;t.integrity=Math.max(0,t.integrity-damage);const d=b.velocity.clone().normalize();if(!invulnerable)t.body.applyImpulse({x:d.x*.75,y:.08,z:d.z*.75},true);burst(p.x,p.y,p.z,.45,POWER_META.blaster.color);if(t.integrity<=0)eliminate(t,"BLASTED");hit=true;break}if(hit){scene.remove(b.mesh);b.mesh.geometry.dispose();(b.mesh.material as import("three").Material).dispose();projectiles.splice(i,1)}}}
