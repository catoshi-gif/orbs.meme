...
type Pickup = { mesh:import("three").Object3D; x:number; y:number; z:number; kind:PickupKind; readyAt:number; baseY:number; blockedBy:Set<string> };
type Pedestal = { mesh:import("three").Object3D; x:number; z:number; kind:ArenaPowerKind; readyAt:number; glow:import("three").Mesh; badge:import("three").Sprite; blockedBy:Set<string> };
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      function addPedestal(x:number,z:number,kind:ArenaPowerKind){
        ...
        return {mesh,x,z,kind,readyAt:0,glow,badge,blockedBy:new Set<string>()} satisfies Pedestal;
      }
      ...
      function addPickup(rx:number,rz:number,surfaceY:number,kind:PickupKind){
        ...
        const y=surfaceY+.16;group.position.set(rx*s,y,rz*s);scene.add(group);pickups.push({mesh:group,x:rx*s,y,z:rz*s,kind,readyAt:0,baseY:y,blockedBy:new Set<string>()});
      }
      ...
      function updatePickups(now:number,matchElapsed:number){const respawnFactor=matchElapsed>=config.overchargeAt ? 0.38 : matchElapsed>=config.suddenDeathAt ? 0.58 : 1;for(const pickup of pickups){pickup.mesh.rotation.y+=.018;pickup.mesh.position.y=pickup.baseY+Math.abs(Math.sin(now*.003+pickup.x))*.08;const ready=now>=pickup.readyAt;pickup.mesh.visible=ready;for(const o of orbs){const p=o.body.translation(),inside=Math.hypot(p.x-pickup.x,p.z-pickup.z)<=1.5*s&&Math.abs(p.y-pickup.y)<=2.4;if(!inside){pickup.blockedBy.delete(o.id);continue}if(!ready||!o.alive||pickup.blockedBy.has(o.id))continue;pickup.readyAt=now+config.ringRespawnMs*respawnFactor;pickup.blockedBy.add(o.id);pickup.mesh.visible=false;if(pickup.kind==="recovery"){const before=o.integrity;o.integrity=Math.min(100,o.integrity+config.recoveryAmount);audio.recover();if(o.isHuman){setIntegrity(Math.round(o.integrity));setEventText(`HEALTH +${Math.round(o.integrity-before)}`)}}else grantPower(o,pickup.kind,now);break}}
        for(const ped of pedestals){ped.glow.rotation.z+=.025;const ready=now>=ped.readyAt;ped.glow.visible=ready;ped.badge.visible=ready;if(ped.glow.userData.halo)ped.glow.userData.halo.visible=ready;if(ped.glow.userData.beacon)ped.glow.userData.beacon.visible=ready;if(ped.glow.userData.crown)ped.glow.userData.crown.visible=ready;if(ped.glow.userData.light)ped.glow.userData.light.visible=ready;for(const o of orbs){const p=o.body.translation(),inside=Math.hypot(p.x-ped.x,p.z-ped.z)<=1.55*s&&p.y>=1.35;if(!inside){ped.blockedBy.delete(o.id);continue}if(!ready||!o.alive||o.powerKind||ped.blockedBy.has(o.id))continue;ped.readyAt=now+config.pedestalRespawnMs*respawnFactor;ped.blockedBy.add(o.id);ped.glow.visible=false;ped.badge.visible=false;if(ped.glow.userData.halo)ped.glow.userData.halo.visible=false;if(ped.glow.userData.beacon)ped.glow.userData.beacon.visible=false;if(ped.glow.userData.crown)ped.glow.userData.crown.visible=false;if(ped.glow.userData.light)ped.glow.userData.light.visible=false;grantPower(o,ped.kind,now);break}}
      }
