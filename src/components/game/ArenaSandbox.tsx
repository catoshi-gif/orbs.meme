...
export default function ArenaSandbox({playerCount,style,seed,pace,generation}:Props){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const bulletGeometry=new THREE.SphereGeometry(.16*s,12,8);
      const bulletMaterial=new THREE.MeshBasicMaterial({color:POWER_META.blaster.color});
      ...
      const acquireBullet=()=>{const mesh=bulletPool.pop()??makeBulletMesh();mesh.visible=true;return mesh};
      const releaseBullet=(mesh:import("three").Mesh)=>{mesh.visible=false;mesh.position.set(0,-1000,0);bulletPool.push(mesh)};
      ...
      function fireBullet(o:OrbSim,now:number){const dir=facingFor(o),p=o.body.translation(),mesh=acquireBullet();const pos=new THREE.Vector3(p.x+dir.x*.95,p.y+.06,p.z+dir.z*.95);mesh.position.copy(pos);projectiles.push({mesh,position:pos,velocity:dir.multiplyScalar(14),owner:o,born:now,life:1200});audio.blasterShot()}
      function updateProjectiles(now:number,dt:number){for(const o of orbs){if(o.alive&&o.blasterActive&&o.powerKind==="blaster"&&now<o.powerExpiresAt&&now>=o.nextShotAt){fireBullet(o,now);o.nextShotAt=now+145}}for(let i=projectiles.length-1;i>=0;i--){const b=projectiles[i]!,age=now-b.born;if(age>b.life){releaseBullet(b.mesh);projectiles.splice(i,1);continue}b.position.addScaledVector(b.velocity,dt);b.mesh.position.copy(b.position);let hit=false;for(const t of orbs){if(!t.alive||t===b.owner)continue;const p=t.body.translation();if(Math.hypot(p.x-b.position.x,p.y-b.position.y,p.z-b.position.z)>PHYSICS.ballRadius+.22*s)continue;const invulnerable=now<t.speedUntil,damage=invulnerable?0:8;t.integrity=Math.max(0,t.integrity-damage);const d=b.velocity.clone().normalize();if(!invulnerable)t.body.applyImpulse({x:d.x*.75,y:.08,z:d.z*.75},true);burst(p.x,p.y,p.z,.45,POWER_META.blaster.color);if(t.integrity<=0)eliminate(t,"BLASTED");hit=true;break}if(hit){releaseBullet(b.mesh);projectiles.splice(i,1)}}}
