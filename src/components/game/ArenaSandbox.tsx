...
type OrbSim = {
  ...
  slamArmed:boolean; slamStartedAt:number; blasterActive:boolean; nextShotAt:number; explosionPrimedUntil:number; doubleJumpArmed:boolean;
};

type Shockwave = { mesh:import("three").Mesh; born:number; life:number };
type Projectile = { mesh:import("three").Mesh; position:import("three").Vector3; velocity:import("three").Vector3; owner:OrbSim; born:number; life:number };
...
const POWER_META:Record<ArenaPowerKind,{label:string;color:string}> = {
  ...
  explosion:{label:"FIRE BURST",color:"#FF4D38"},
};
const PICKUP_COLOR:Record<PickupKind,string>={kinetic:"#5EF7FF",slam:"#B56CFF",shield:"#7CFFB2",superjump:"#FFD86B",blaster:"#FF8A4C",explosion:"#FF4D38",recovery:"#FF6FAE"};
...
export default function ArenaSandbox({playerCount,style,seed,pace,generation}:Props){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      function addPedestal(x:number,z:number,accent:string){
        ...
        const beacon=new THREE.Mesh(new THREE.CylinderGeometry(.08*s,.42*s,3.8,24,1,true),new THREE.MeshBasicMaterial({color:accent,transparent:true,opacity:.065,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,depthWrite:false}));beacon.position.set(x,3.22,z);scene.add(beacon);
        glow.userData.halo=halo;glow.userData.beacon=beacon;
        return {mesh,x,z,readyAt:0,glow} satisfies Pedestal;
      }
      ...
      const pedestalPalette=[POWER_META.kinetic.color,POWER_META.slam.color,POWER_META.shield.color,POWER_META.superjump.color,POWER_META.blaster.color,POWER_META.explosion.color];
      const pedestals:Pedestal[]=[[-14,-7],[14,-7],[-14,7],[14,7],[0,-15],[0,15]].map(([xx,zz],i)=>addPedestal(xx*s,zz*s,pedestalPalette[i%pedestalPalette.length]!));
      ...
      function addPickup(rx:number,rz:number,surfaceY:number,kind:PickupKind){
        ...
        if(kind==="explosion"){
          const fireMat=new THREE.MeshBasicMaterial({color:"#FFD15C"});
          const core=new THREE.Mesh(new THREE.IcosahedronGeometry(.34*s,1),fireMat);core.position.y=1.45;group.add(core);
        }
        const y=surfaceY+.12;group.position.set(rx*s,y,rz*s);scene.add(group);pickups.push({mesh:group,x:rx*s,y,z:rz*s,kind,readyAt:0,baseY:y});
      }
      // All free pickups sit on clear flat courts; elevated pickups sit only on known dais levels.
      addPickup(-20,-5,0,"kinetic"); addPickup(20,5,0,"slam");
      addPickup(-5,-20,0,"shield"); addPickup(5,20,0,"superjump");
      addPickup(-17,15,0,"recovery"); addPickup(17,-15,0,"recovery");
      addPickup(-16,-15,0,"blaster"); addPickup(16,15,0,"explosion");
      addPickup(-4.2,0,1.61,"kinetic"); addPickup(4.2,0,1.61,"superjump");
      addPickup(0,-4.2,1.61,"blaster"); addPickup(0,4.2,1.61,"explosion");
      ...
      const clearPower=(o:OrbSim)=>{o.powerKind=null;o.powerExpiresAt=0;o.blasterActive=false;o.explosionPrimedUntil=0;o.doubleJumpArmed=false};
      ...
      const useAction=(o:OrbSim)=>{if(!live||!o.alive)return;const now=performance.now();if(o.powerKind&&now>=o.powerExpiresAt)clearPower(o);const kind=o.powerKind;if(!kind){normalJump(o);return}
        if(kind==="kinetic"){const v=o.body.linvel(),speed=Math.hypot(v.x,v.z),dx=speed>.3?v.x/speed:(o.isHuman?lastFacing.x:Math.cos(o.botHeading)),dz=speed>.3?v.z/speed:(o.isHuman?lastFacing.z:Math.sin(o.botHeading));o.body.applyImpulse({x:dx*3.1,y:.18,z:dz*3.1},true);o.kineticUntil=now+1250;audio.powerUse(kind);clearPower(o)}
          ...
          if(grounded(o)){if(normalJump(o,1.15)){o.doubleJumpArmed=true;audio.powerUse(kind);if(o.isHuman)setEventText("DOUBLE JUMP · PRESS AGAIN IN AIR")}}
          else if(o.doubleJumpArmed){const v=o.body.linvel();o.body.setLinvel({x:v.x,y:config.jumpImpulse*1.32,z:v.z},true);o.doubleJumpArmed=false;audio.powerUse(kind);clearPower(o);if(o.isHuman)setEventText("DOUBLE JUMP")}
        }
        else if(kind==="slam"){if(normalJump(o,1.55)){o.slamArmed=true;o.slamStartedAt=now;audio.powerUse(kind);clearPower(o)}}
        else if(kind==="blaster"){o.blasterActive=true;o.nextShotAt=0;audio.powerUse(kind);if(o.isHuman)setEventText("BLASTER ACTIVE · 5 SECONDS")}
        else if(kind==="explosion"){
          if(now<o.explosionPrimedUntil){fireExplosion(o);audio.explosion();clearPower(o);if(o.isHuman)setEventText("FIRE BURST · BOOM")}
          else{o.explosionPrimedUntil=now+1250;audio.powerUse(kind);if(o.isHuman)setEventText("FIRE BURST PRIMED · PRESS AGAIN")}
        }
      ...
      function fireBullet(o:OrbSim,now:number){const dir=facingFor(o),p=o.body.translation(),mesh=new THREE.Mesh(new THREE.SphereGeometry(.16*s,12,8),new THREE.MeshBasicMaterial({color:POWER_META.blaster.color}));const pos=new THREE.Vector3(p.x+dir.x*.95,p.y+.06,p.z+dir.z*.95);mesh.position.copy(pos);scene.add(mesh);projectiles.push({mesh,position:pos,velocity:dir.multiplyScalar(14),owner:o,born:now,life:1200});audio.blasterShot()}
      function fireExplosion(o:OrbSim){const p=o.body.translation();burst(p.x,p.y,p.z,1.35,POWER_META.explosion.color);for(const t of orbs){if(!t.alive||t===o)continue;const tp=t.body.translation(),dx=tp.x-p.x,dz=tp.z-p.z,dist=Math.hypot(dx,dz);if(dist>5.2*s||dist<.01)continue;const strength=1-dist/(5.2*s),shielded=performance.now()<t.shieldUntil,damage=(12+18*strength)*(shielded?.3:1);t.integrity=Math.max(0,t.integrity-damage);t.body.applyImpulse({x:dx/dist*(1.8+3.2*strength),y:.55+strength*.8,z:dz/dist*(1.8+3.2*strength)},true);if(t.integrity<=0)eliminate(t,"BURNED OUT")}if(o.isHuman)setIntegrity(Math.round(o.integrity))}
      function updateProjectiles(now:number,dt:number){for(const o of orbs){if(o.alive&&o.blasterActive&&o.powerKind==="blaster"&&now<o.powerExpiresAt&&now>=o.nextShotAt){fireBullet(o,now);o.nextShotAt=now+145}}for(let i=projectiles.length-1;i>=0;i--){const b=projectiles[i]!,age=now-b.born;if(age>b.life){scene.remove(b.mesh);b.mesh.geometry.dispose();(b.mesh.material as import("three").Material).dispose();projectiles.splice(i,1);continue}b.position.addScaledVector(b.velocity,dt);b.mesh.position.copy(b.position);let hit=false;for(const t of orbs){if(!t.alive||t===b.owner)continue;const p=t.body.translation();if(Math.hypot(p.x-b.position.x,p.y-b.position.y,p.z-b.position.z)>PHYSICS.ballRadius+.22*s)continue;const shielded=now<t.shieldUntil,damage=shielded?2.4:8;t.integrity=Math.max(0,t.integrity-damage);const d=b.velocity.clone().normalize();t.body.applyImpulse({x:d.x*.75,y:.08,z:d.z*.75},true);burst(p.x,p.y,p.z,.45,POWER_META.blaster.color);if(t.integrity<=0)eliminate(t,"BLASTED");hit=true;break}if(hit){scene.remove(b.mesh);b.mesh.geometry.dispose();(b.mesh.material as import("three").Material).dispose();projectiles.splice(i,1)}}}
      ...
      function updatePickups(now:number,matchElapsed:number){const respawnFactor=matchElapsed>=config.overchargeAt ? 0.38 : matchElapsed>=config.suddenDeathAt ? 0.58 : 1;for(const pickup of pickups){pickup.mesh.rotation.y+=.018;pickup.mesh.position.y=pickup.baseY+Math.abs(Math.sin(now*.003+pickup.x))*.08;const ready=now>=pickup.readyAt;pickup.mesh.visible=ready;if(!ready)continue;for(const o of orbs){if(!o.alive)continue;const p=o.body.translation();if(Math.hypot(p.x-pickup.x,p.z-pickup.z)>1.5*s||Math.abs(p.y-pickup.y)>2.4)continue;pickup.readyAt=now+config.ringRespawnMs*respawnFactor;pickup.mesh.visible=false;if(pickup.kind==="recovery"){const before=o.integrity;o.integrity=Math.min(100,o.integrity+config.recoveryAmount);audio.recover();if(o.isHuman){setIntegrity(Math.round(o.integrity));setEventText(`HEALTH +${Math.round(o.integrity-before)}`)}}else grantPower(o,pickup.kind,now);break}}
        for(const ped of pedestals){ped.glow.rotation.z+=.025;const ready=now>=ped.readyAt;ped.glow.visible=ready;if(ped.glow.userData.halo)ped.glow.userData.halo.visible=ready;if(ped.glow.userData.beacon)ped.glow.userData.beacon.visible=ready;if(!ready)continue;for(const o of orbs){if(!o.alive||o.powerKind)continue;const p=o.body.translation();if(Math.hypot(p.x-ped.x,p.z-ped.z)>1.55*s||p.y<1.35)continue;const kinds:ArenaPowerKind[]=["kinetic","slam","shield","superjump","blaster","explosion"];const kind=kinds[Math.floor(rand()*kinds.length)]!;ped.readyAt=now+config.pedestalRespawnMs*respawnFactor;ped.glow.visible=false;if(ped.glow.userData.halo)ped.glow.userData.halo.visible=false;if(ped.glow.userData.beacon)ped.glow.userData.beacon.visible=false;grantPower(o,kind,now);break}}
      }
      function botInput(o:OrbSim,nowSec:number,nowMs:number){const p=o.body.translation();if(o.powerKind&&nowMs<o.powerExpiresAt){let nearest=99;for(const e of orbs){if(e===o||!e.alive)continue;const ep=e.body.translation();nearest=Math.min(nearest,Math.hypot(ep.x-p.x,ep.z-p.z))}if(o.powerKind==="explosion"&&nowMs<o.explosionPrimedUntil)useAction(o);else if(nearest<4.5&&rand()<.08)useAction(o)}if(nowSec>=o.botThinkAt){let tx=0,tz=0,best=Infinity;const candidates:{x:number;z:number;score:number}[]=[];for(const pick of pickups){if(nowMs<pick.readyAt)continue;if(pick.kind==="recovery"&&o.integrity>62)continue;const dd=Math.hypot(pick.x-p.x,pick.z-p.z);candidates.push({x:pick.x,z:pick.z,score:dd*(pick.kind==="recovery" ? .62 : 1)})}for(const ped of pedestals){if(nowMs<ped.readyAt)continue;const dd=Math.hypot(ped.x-p.x,ped.z-p.z);candidates.push({x:ped.x,z:ped.z,score:dd*.9})}for(const c of candidates)if(c.score<best){best=c.score;tx=c.x;tz=c.z}if(!candidates.length||o.powerKind){const enemies=orbs.filter(v=>v.alive&&v!==o);let target=enemies[0];best=Infinity;for(const e of enemies){const ep=e.body.translation(),dd=Math.hypot(ep.x-p.x,ep.z-p.z);if(dd<best){best=dd;target=e}}if(target){const tp=target.body.translation();tx=tp.x;tz=tp.z}}o.botHeading=Math.atan2(tz-p.z,tx-p.x)+(rand()-.5)*(o.personality==="hunter" ? .18 : .45);o.botThinkAt=nowSec+.18+rand()*.32;if(rand()<.055)normalJump(o)}return{x:Math.cos(o.botHeading),y:-Math.sin(o.botHeading)}}
  ...
  return <div className="arena-sandbox-game arena-course-game">
    ...
    {phase==="ready"?<div className="arena-center-card"><span>ADMIN ARENA V5</span><h3>POWER COLOSSEUM</h3><p>Fight for temporary weapons, jump onto power pedestals, use the multi-level Orb dais, and grab pink recovery rings for a second chance. Cyan = kinetic · purple = slam · green = shield · gold ↑ = double jump · orange = blaster · red = fire burst · pink/green + = health.</p><button className="btn-primary" onClick={()=>startRef.current()}>Enter Arena →</button></div>:null}
    ...
    <div className="arena-control-hint"><span className="arena-desktop-hint">ARROWS / WASD · ROLL &nbsp;&nbsp; SPACE · {powerMeta?"USE POWER":"JUMP"}</span><span className="arena-mobile-hint">ROLL · {powerMeta?"USE POWER":"JUMP"} · FIGHT FOR RINGS · PINK + = HEALTH</span></div>
  </div>;
}
