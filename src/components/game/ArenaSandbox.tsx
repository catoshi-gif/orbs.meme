...
type OrbSim = {
  ...
  blasterActive:boolean; nextShotAt:number; doubleJumpArmed:boolean;
...
const PICKUP_COLOR:Record<PickupKind,string>={superjump:"#FFD86B",blaster:"#FF8A4C",superspeed:"#72F7FF",recovery:"#FF6FAE"};
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      function addPedestal(x:number,z:number,kind:ArenaPowerKind){
        ...
        const badge=makeLabel(THREE,kind==="superjump"?"↑ DOUBLE JUMP":kind==="blaster"?"● BLASTER":"» SUPER SPEED",accent);badge.position.set(x,5.15,z);badge.scale.multiplyScalar(.95);scene.add(badge);
      ...
      const spikeHazards:{axis:"x";cx:number;cz:number;half:number;dir:1|-1}[]=[
      ...
      for(const [cx,dir] of [[-d,1],[d,-1]] as const)for(const side of [-1,1])for(let j=0;j<8;j++){const x=cx-4.25*s+j*(8.5*s/7),z=side*3.25*s,rampT=clamp(dir>0?(x-(cx-5*s))/(10*s):((cx+5*s)-x)/(10*s),0,1),y=rampT*2.2*s+.24*s;spikeMatrix.makeTranslation(x,y,z);spikeMesh.setMatrixAt(spikeIndex++,spikeMatrix)}
      ...
      const pedestalKinds:ArenaPowerKind[]=["superjump","blaster","superspeed","superjump","blaster","superspeed"];
      ...
      const bulletGeometry=new THREE.TorusGeometry(.34*s,.085*s,6,18,Math.PI*1.22);
      const bulletMaterial=new THREE.MeshBasicMaterial({color:POWER_META.blaster.color,transparent:true,opacity:.9,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,depthWrite:false});
      const bulletPool:import("three").Mesh[]=[];
      const makeBulletMesh=()=>{const mesh=new THREE.Mesh(bulletGeometry,bulletMaterial);mesh.visible=false;scene.add(mesh);return mesh};
      for(let i=0;i<64;i++)bulletPool.push(makeBulletMesh());
      const acquireBullet=()=>{const mesh=bulletPool.pop()??makeBulletMesh();mesh.visible=true;mesh.scale.setScalar(1);return mesh};
      const releaseBullet=(mesh:import("three").Mesh)=>{mesh.visible=false;mesh.position.set(0,-1000,0);mesh.scale.setScalar(1);bulletPool.push(mesh)};
      ...
      function fireBullet(o:OrbSim,now:number){const dir=facingFor(o),p=o.body.translation(),mesh=acquireBullet();const pos=new THREE.Vector3(p.x+dir.x*.95,p.y+.06,p.z+dir.z*.95);mesh.position.copy(pos);mesh.rotation.set(Math.PI/2,0,Math.atan2(dir.z,dir.x));projectiles.push({mesh,position:pos,velocity:dir.multiplyScalar(13.5),owner:o,born:now,life:1700});audio.blasterShot()}
      function updateProjectiles(now:number,dt:number){for(const o of orbs){if(o.alive&&o.blasterActive&&o.powerKind==="blaster"&&now<o.powerExpiresAt&&now>=o.nextShotAt){fireBullet(o,now);o.nextShotAt=now+180}}for(let i=projectiles.length-1;i>=0;i--){const b=projectiles[i]!,age=now-b.born;if(age>b.life){releaseBullet(b.mesh);projectiles.splice(i,1);continue}b.position.addScaledVector(b.velocity,dt);b.mesh.position.copy(b.position);b.mesh.scale.setScalar(1+Math.min(.34,age/b.life*.34));let hit=false;for(const t of orbs){if(!t.alive||t===b.owner)continue;const p=t.body.translation();const hitRadius=PHYSICS.ballRadius+clamp(.58*s,.52,.9);if(Math.hypot(p.x-b.position.x,p.y-b.position.y,p.z-b.position.z)>hitRadius)continue;const invulnerable=now<t.speedUntil,damage=invulnerable?0:8;t.integrity=Math.max(0,t.integrity-damage);t.damageFlashUntil=now+155;const d=b.velocity.clone().normalize();if(!invulnerable)t.body.applyImpulse({x:d.x*.75,y:.08,z:d.z*.75},true);burst(p.x,p.y,p.z,.45,POWER_META.blaster.color);if(t.integrity<=0)eliminate(t,"BLASTED");hit=true;break}if(hit){releaseBullet(b.mesh);projectiles.splice(i,1)}}}
      ...
      const render=(ms:number)=>{if(cancelled)return;frame=requestAnimationFrame(render);const dt=Math.min(.05,(ms-prev)/1000);prev=ms;const matchElapsed=live?(ms-matchStart)/1000:0;if(live&&!resolved){setElapsed(matchElapsed);const alive=orbs.filter(o=>o.alive).length;audio.setIntensity(clamp(Math.max(config.maxSeconds>0?matchElapsed/config.maxSeconds:0,1-alive/playerCount),0,1));if(matchElapsed>config.overchargeAt)setEventText("OVERCHARGE · POWER RINGS RECHARGE FAST");else if(matchElapsed>config.suddenDeathAt)setEventText("POWER SURGE · RINGS RECHARGE FASTER");acc+=dt;while(acc>=PHYSICS.fixedStep){simTime+=PHYSICS.fixedStep;for(const o of orbs){if(!o.alive)continue;let ix=0,iy=0;if(o.isHuman){const k=controlsRef.current;let sx=(k.right?1:0)-(k.left?1:0),sy=(k.up?1:0)-(k.down?1:0);if(controlModeRef.current==="touch"){sx=k.touchX;sy=k.touchY}const rightX=-lastFacing.z,rightZ=lastFacing.x,worldX=rightX*sx+lastFacing.x*sy,worldZ=rightZ*sx+lastFacing.z*sy;ix=worldX;iy=-worldZ;if(Math.hypot(worldX,worldZ)>.2){const desiredHeading=new THREE.Vector3(worldX,0,worldZ).normalize();lastFacing.lerp(desiredHeading,.026).normalize()}}else{const b=botInput(o,simTime,performance.now());ix=b.x;iy=b.y}const v=o.body.linvel(),profile=o.isHuman&&controlModeRef.current==="keys"?"desktop":"mobile",speedMul=performance.now()<o.speedUntil?5:1,steerBase=nextPlanarVelocity({x:v.x/speedMul,z:v.z/speedMul},ix,iy,profile),steer={x:steerBase.x*speedMul,z:steerBase.z*speedMul};o.body.setLinvel({x:steer.x,y:v.y,z:steer.z},true)}world.step();const now=performance.now();handleImpacts(now);updatePickups(now,matchElapsed);updateProjectiles(now,PHYSICS.fixedStep);for(const o of orbs){if(!o.alive)continue;const v=o.body.linvel(),profile=o.isHuman&&controlModeRef.current==="keys"?"desktop":"mobile",speedMul=now<o.speedUntil?5:1,baseClamp=clampPlanarSpeed({x:v.x/speedMul,z:v.z/speedMul},profile),cl={x:baseClamp.x*speedMul,z:baseClamp.z*speedMul};if(cl.x!==v.x||cl.z!==v.z)o.body.setLinvel({x:cl.x,y:v.y,z:cl.z},true);const p=o.body.translation();if(o.airborne&&Math.abs(v.y)<.55)o.airborne=false;if(o.powerKind&&now>=o.powerExpiresAt){clearPower(o);if(o.isHuman){setPower(null);setPowerLeft(0);setEventText("POWER EXPIRED · FIND ANOTHER")}}if(p.y<-3.6){eliminate(o,"FELL");continue}
            if(now>=o.hazardReadyAt){
              let hazardHit=false;
              if(Math.hypot(p.x,p.z)>arenaHalf*.84)for(const h of columnHazards){const dx=p.x-h.x,dz=p.z-h.z;if(Math.hypot(dx,dz)>h.r+PHYSICS.ballRadius*.9)continue;hazardHit=true;o.hazardReadyAt=now+850;const invulnerable=now<o.speedUntil;if(!invulnerable){o.integrity=Math.max(0,o.integrity-3);o.damageFlashUntil=now+155;const mag=Math.max(.001,Math.hypot(p.x,p.z));o.body.applyImpulse({x:-p.x/mag*.72,y:.12,z:-p.z/mag*.72},true);audio.zap();if(o.integrity<=0)eliminate(o,"ZAPPED")}break}
              if(!hazardHit&&now>=o.hazardReadyAt)for(const h of spikeHazards){const rampT=clamp(h.dir>0?(p.x-(h.cx-5*s))/(10*s):((h.cx+5*s)-p.x)/(10*s),0,1),rampY=rampT*2.2*s;if(Math.abs(p.x-h.cx)>h.half+PHYSICS.ballRadius||Math.abs(p.z-h.cz)>.46*s+PHYSICS.ballRadius||p.y>rampY+.9*s)continue;o.hazardReadyAt=now+850;const invulnerable=now<o.speedUntil;if(!invulnerable){o.integrity=Math.max(0,o.integrity-4);o.damageFlashUntil=now+155;const side=p.z>=h.cz?1:-1;o.body.applyImpulse({x:0,y:.12,z:side*.72},true);audio.bump(.72);if(o.integrity<=0)eliminate(o,"SPIKED")}break}
