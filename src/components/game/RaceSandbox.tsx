...
export default function RaceSandbox({playerCount,style,seed,generation}:Props){
  ...
  const touchMove=useCallback((e:React.PointerEvent<HTMLDivElement>)=>{if(!e.currentTarget.hasPointerCapture(e.pointerId))return;const r=e.currentTarget.getBoundingClientRect(),rawX=(e.clientX-(r.left+r.width/2))/(r.width*.38),rawY=(e.clientY-(r.top+r.height/2))/(r.height*.38),mag=Math.max(1,Math.hypot(rawX,rawY)),x=clamp(rawX/mag,-1,1),y=clamp(rawY/mag,-1,1);controlsRef.current.touchX=x;controlsRef.current.up=y<-.16;controlsRef.current.down=y>.30;e.currentTarget.style.setProperty("--race-stick-x",`${x*31}px`);e.currentTarget.style.setProperty("--race-stick-y",`${y*31}px`)},[]);
  const touchStart=useCallback((e:React.PointerEvent<HTMLDivElement>)=>{e.currentTarget.setPointerCapture(e.pointerId);const r=e.currentTarget.getBoundingClientRect(),rawX=(e.clientX-(r.left+r.width/2))/(r.width*.38),rawY=(e.clientY-(r.top+r.height/2))/(r.height*.38),mag=Math.max(1,Math.hypot(rawX,rawY)),x=clamp(rawX/mag,-1,1),y=clamp(rawY/mag,-1,1);controlsRef.current.touchX=x;controlsRef.current.up=y<-.16;controlsRef.current.down=y>.30;e.currentTarget.style.setProperty("--race-stick-x",`${x*31}px`);e.currentTarget.style.setProperty("--race-stick-y",`${y*31}px`)},[]);
  const touchEnd=useCallback((e:React.PointerEvent<HTMLDivElement>)=>{controlsRef.current.touchX=0;controlsRef.current.up=false;controlsRef.current.down=false;e.currentTarget.style.setProperty("--race-stick-x","0px");e.currentTarget.style.setProperty("--race-stick-y","0px");try{e.currentTarget.releasePointerCapture(e.pointerId)}catch{}},[]);
  ...
  return <div className="race-sandbox-game">
    ...
    {phase==="playing"&&controlMode==="touch"?<div className="race-touch" onPointerDown={touchStart} onPointerMove={touchMove} onPointerUp={touchEnd} onPointerCancel={touchEnd}><i/><span>GAS</span><b>← STEER →</b></div>:null}
    <div className="arena-control-hint race-control-hint"><span className="arena-desktop-hint">HOLD W / ↑ · ACCELERATE &nbsp;&nbsp; A / D OR ← / → · STEER &nbsp;&nbsp; S / ↓ · BRAKE &nbsp;&nbsp; SPACE · JUMP{item?" + USE ITEM":""}</span><span className="arena-mobile-hint">JOYSTICK ↑ GAS · ← → STEER · ↓ BRAKE · JUMP{item?" + USE ITEM":""} · DRAFT · BOOST · 3 LAPS</span></div>
  </div>;
}
