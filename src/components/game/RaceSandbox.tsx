...
export default function RaceSandbox({playerCount,style,seed,generation}:Props){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const camPos=new THREE.Vector3(),camLook=new THREE.Vector3(),forward=new THREE.Vector3(),rightV=new THREE.Vector3(),desired=new THREE.Vector3(),smoothForward=spawnT.clone(),targetLook=new THREE.Vector3(),cameraMatrix=new THREE.Matrix4(),cameraTargetQuat=new THREE.Quaternion();let perfFrames=0,perfWindowStart=performance.now();
      const render=(ms:number)=>{if(cancelled)return;frame=requestAnimationFrame(render);const dt=Math.min(.05,(ms-prev)/1000);prev=ms;(trackMat.uniforms.uTime.value as number)=ms;ring.rotation.y+=dt*.018;perfFrames+=1;if(ms-perfWindowStart>2200){const fps=perfFrames*1000/Math.max(1,ms-perfWindowStart),next=fps<50?Math.max(.82,renderDpr-.12):fps>58?Math.min(maxDpr,renderDpr+.05):renderDpr;if(Math.abs(next-renderDpr)>.02){renderDpr=next;renderer.setPixelRatio(renderDpr);resize()}perfFrames=0;perfWindowStart=ms;}for(const visual of pickupVisuals.values()){const g=visual.group;if(ms<visual.hiddenUntil){g.visible=false;continue}if(!g.visible){g.visible=true;g.scale.setScalar(.08)}const respawnScale=1-Math.exp(-dt*12);g.scale.lerp(new THREE.Vector3(1,1,1),respawnScale);g.rotation.z+=dt*.78;(g.children[1] as import("three").Mesh).rotation.y+=dt*2.8;(g.children[2] as import("three").Mesh).rotation.z-=dt*1.7;g.position.y+=Math.sin(ms*.0022+visual.phase)*.0009}for(const b of boostMeshes)b.material instanceof THREE.MeshBasicMaterial&&(b.material.opacity=.62+Math.sin(ms*.006+b.position.x)*.18);
        if(live){const now=performance.now(),elapsedNow=(now-matchStart)/1000;acc+=dt;while(acc>=PHYSICS.fixedStep){simTime+=PHYSICS.fixedStep;for(const o of racers){if(o.finishedAt)continue;if(o.recovering){if(now>=o.rescueUntil)finishRescue(o);continue}const p=o.body.translation(),nearest=nearestRacePoint(manifest.points,p.x,p.y,p.z,o.pointIndex),rp=nearest.point;o.previousPointIndex=o.pointIndex;o.pointIndex=rp.index;if(o.pointIndex>manifest.points.length*.24)o.lapArmed=true;if(o.lapArmed&&isForwardLapWrap(o.previousPointIndex,o.pointIndex,manifest.points.length)){o.lap+=1;o.lapArmed=false;if(o.isHuman){audio.lap();setLap(Math.min(config.laps,o.lap+1));setEventText(o.lap>=config.laps-1?"FINAL LAP":"LAP COMPLETE")};if(o.lap>=config.laps){o.finishedAt=now;o.body.setLinvel({x:0,y:o.body.linvel().y,z:0},true);if(!resolved){resolved=true;setWinner(o.username);if(o.isHuman){setRacePhase("won");audio.victory()}else setEventText(`${o.username} WINS · FINISH YOUR RACE`)}else if(o.isHuman){setRacePhase("finished")}continue}}
          ...
          updateItems(o,now);const k=controlsRef.current;const rawHumanSteer=coarse?k.touchX:(k.left?-1:0)+(k.right?1:0),steer=o.isHuman?-rawHumanSteer:botSteer(o,now);const v=o.body.linvel(),air=!grounded(o),slow=now < o.slowedUntil ? .72 : 1,boost=now<o.boostUntil,throttle=o.isHuman?(k.up?1:0):1,brake=o.isHuman&&k.down;let targetSpeed=throttle?(boost?config.boostSpeed:config.baseSpeed):0;if(throttle)targetSpeed+=Math.max(0,-rp.tangentY)*13;if(brake)targetSpeed*=.22;targetSpeed*=slow;forward.set(rp.tangentX,0,rp.tangentZ).normalize();rightV.set(rp.rightX,0,rp.rightZ).normalize();desired.copy(forward).addScaledVector(rightV,steer*config.steeringStrength).normalize();const currentSpeed=Math.hypot(v.x,v.z),response=air ? (throttle ? .048 : .012) : (throttle ? .115 : .16),desiredSpeed=clamp(targetSpeed,0,boost?config.boostSpeed:config.maxSpeed+Math.max(0,-rp.tangentY)*12);if(now<o.plungeLaunchLockUntil){/* preserve authored hero takeoff while the rigid body clears the ramp */}else if(now<o.plungeAirUntil&&air){/* preserve ballistic horizontal velocity; Rapier gravity owns Y */}else{const tx=desired.x*desiredSpeed,tz=desired.z*desiredSpeed;let nx=v.x+(tx-v.x)*response,nz=v.z+(tz-v.z)*response;if(throttle&&currentSpeed>desiredSpeed*1.08&&desiredSpeed>0){const f=desiredSpeed/currentSpeed;nx*=f;nz*=f}o.body.setLinvel({x:nx,y:v.y,z:nz},true)}}world.step();updateProjectiles(now,PHYSICS.fixedStep);acc-=PHYSICS.fixedStep}updateDraft(now);if(ms-lastUi>90){lastUi=ms;const v=human.body.linvel();setElapsed(elapsedNow);setSpeed(Math.hypot(v.x,v.z));const currentPlace=rankOf(human);setPlace(currentPlace);if(currentPlace<lastHumanPlace&&lastHumanPlace-currentPlace<=3&&elapsedNow>3){audio.overtake();setEventText(`OVERTAKE · P${currentPlace}`)}lastHumanPlace=currentPlace;setItem(human.item);if(human.recovering)setRescueLeft(Math.max(0,(human.rescueUntil-now)/1000));else setRescueLeft(0);audio.setSpeed(Math.hypot(v.x,v.z))}}
        for(const o of racers){const mat=o.mesh.material as import("three").ShaderMaterial;mat.uniforms.uTime.value=ms;if(o.recovering){const t=clamp((ms-o.rescueStartedAt)/RACE_RESCUE_MS,0,1),rp=manifest.points[o.rescuePointIndex]!,end=pointPosition(THREE,rp,0,1.25),arc=Math.sin(t*Math.PI)*5.2;o.mesh.position.lerpVectors(o.rescueFrom,end,t);o.mesh.position.y+=arc;o.label.position.set(o.mesh.position.x,o.mesh.position.y-.22,o.mesh.position.z);o.cloud.visible=true;o.cloud.position.copy(o.mesh.position);o.cloud.position.y-=.68;o.cloud.rotation.y+=dt*1.2;continue}o.cloud.visible=false;const p=o.body.translation(),q=o.body.rotation(),v=o.body.linvel(),sp=Math.hypot(v.x,v.z);o.mesh.position.set(p.x,p.y,p.z);o.mesh.quaternion.set(q.x,q.y,q.z,q.w);o.label.position.set(p.x,p.y-.22,p.z);mat.uniforms.uSpeed.value=sp;o.core.scale.setScalar(1+clamp((sp-config.baseSpeed)/12,0,.22))}
        const hv=human.body.linvel(),hs=Math.hypot(hv.x,hv.z),hp=human.recovering?human.mesh.position:human.body.translation(),rp=manifest.points[human.pointIndex]!,lookAhead=manifest.points[(human.pointIndex+Math.round(7+clamp(hs,0,25)*.22))%manifest.points.length]!;forward.set(lookAhead.tangentX,0,lookAhead.tangentZ).normalize();smoothForward.lerp(forward,1-Math.exp(-dt*3.4)).normalize();const back=mobileish?11.0:10.0,height=mobileish?5.8:5.25;desired.set(hp.x-smoothForward.x*back,hp.y+height,hp.z-smoothForward.z*back);camPos.lerp(desired,1-Math.exp(-dt*5.2));camera.position.copy(camPos);targetLook.set(hp.x+smoothForward.x*(5.0+hs*.30),hp.y+.52+Math.max(0,lookAhead.tangentY)*1.45,hp.z+smoothForward.z*(5.0+hs*.30));camLook.lerp(targetLook,1-Math.exp(-dt*6.0));cameraMatrix.lookAt(camera.position,camLook,camera.up);cameraTargetQuat.setFromRotationMatrix(cameraMatrix);camera.quaternion.slerp(cameraTargetQuat,1-Math.exp(-dt*8.5));const targetFov=clamp(58+(hs-config.baseSpeed)*1.05,58,71);camera.fov+=(targetFov-camera.fov)*(1-Math.exp(-dt*4.4));camera.updateProjectionMatrix();renderer.render(scene,camera)};
