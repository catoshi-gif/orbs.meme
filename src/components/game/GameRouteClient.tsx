...
export default function GameRouteClient(props: Props) {
  ...
  if (orb) {
    ...
    if (!manifest || !competitiveSession) return <div className="game-route-shell game-jit-state"><div className="game-loader-orb" /><span className="eyebrow">Orb is live</span><h1>{sessionLoading ? "Issuing your session…" : "Preparing the canonical maze…"}</h1><p>{sessionError || "Your X follow, wallet proof and human check are being bound to this exact game manifest."}</p>{sessionError ? <div className="game-ready-actions"><button className="btn-primary" onClick={() => void requestSession()}>Retry entry</button><Link className="btn-secondary" href={`/orb/${props.slug}`}>Qualification</Link></div> : null}</div>;
  }
