...
export default function OrbLobbyHero({ orb, winner = false }: { orb: PublicOrbRecord; winner?: boolean }) {
  ...
  return <section className="card prize-hero real-orb-hero">
    <div className="orb-top"><div className="hostline">{orb.hostX.profileImageUrl ? <img className="host-avatar" src={orb.hostX.profileImageUrl} alt="" referrerPolicy="no-referrer" /> : <span className="avatar" />}Hosted by @{orb.hostX.username}</div><span className={`pill ${live && !closed ? "live" : ""}`}>{closed ? (winner ? "CLEARED" : "EXPIRED") : live ? "LIVE" : "SEALED"}</span></div>
    ...
    {!closed ? <WaitingRoomPresence slug={orb.slug}/> : null}
    {closed ? <div className="live-launch-panel closed"><span>{winner ? "FIRST FINISH VERIFIED" : "COMPETITION ENDED"}</span><strong>{winner ? "The winner lock is secured and the result is ready." : "The six-hour race window closed without a verified winner."}</strong><a className="btn-primary" href={`/orb/${orb.slug}/results`}>View result →</a></div> : live ? <div className="live-launch-panel"><span>THE ORB IS LIVE</span><strong>Same maze. Same start. First verified finish wins.</strong><a className="btn-primary" href="#qualify">Register to enter →</a></div> : <div className="countdown"><div><strong>{String(parts.hours).padStart(2, "0")}</strong><span>Hours</span></div><div><strong>{String(parts.minutes).padStart(2, "0")}</strong><span>Minutes</span></div><div><strong>{String(parts.seconds).padStart(2, "0")}</strong><span>Seconds</span></div><div><strong>{new Date(orb.startsAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</strong><span>Launch</span></div></div>}
    {!closed ? <div className="waiting-game-window">
      <div className="waiting-game-window-head"><span>GAMEPLAY PREVIEW</span><small>Representative maze only. Your sealed maze stays hidden until launch.</small></div>
      <OrbShowcase style={orb.style} compact />
    </div> : null}
