...
export default function OrbLobbyHero({ orb, winner = false }: { orb: PublicOrbRecord; winner?: boolean }) {
  ...
  return <section className="card prize-hero real-orb-hero">
    ...
    {closed ? <div className="live-launch-panel closed"><span>{winner ? "FIRST FINISH VERIFIED" : "COMPETITION ENDED"}</span><strong>{winner ? "The winner lock is secured and the result is ready." : "The six-hour race window closed without a verified winner."}</strong><a className="btn-primary" href={`/orb/${orb.slug}/results`}>View result →</a></div> : live ? <div className="live-launch-panel"><span>THE ORB IS LIVE</span><strong>Same maze. Same start. First verified finish wins.</strong><a className="btn-primary" href="#qualify">Register to enter →</a></div> : <><div className="countdown"><div><strong>{String(parts.hours).padStart(2, "0")}</strong><span>Hours</span></div><div><strong>{String(parts.minutes).padStart(2, "0")}</strong><span>Minutes</span></div><div><strong>{String(parts.seconds).padStart(2, "0")}</strong><span>Seconds</span></div><div><strong>{localLaunchShort || "…"}</strong><span>Launch</span></div></div><div className="local-launch-time"><span>YOUR LOCAL START TIME</span><strong>{localLaunch || "Detecting your local time…"}</strong><small>Set an alarm and come back before launch so you are registered and ready.</small></div></>}
