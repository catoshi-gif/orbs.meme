...
export default function OrbLobbyHero({ orb }: { orb: PublicOrbRecord }) {
  ...
  return <section className="card prize-hero real-orb-hero">
    <div className="orb-top"><div className="hostline">{orb.hostX.profileImageUrl ? <img className="host-avatar" src={orb.hostX.profileImageUrl} alt="" referrerPolicy="no-referrer" /> : <span className="avatar" />}Hosted by @{orb.hostX.username}</div><span className={`pill ${live ? "live" : ""}`}>{live ? "LIVE" : "SEALED TEST"}</span></div>
    ...
    {live ? <div className="live-launch-panel"><span>THE ORB IS LIVE</span><strong>Same maze. Same start. First verified finish wins.</strong><a className="btn-primary" href="#qualify">Qualify to enter →</a></div> : <div className="countdown"><div><strong>{String(parts.hours).padStart(2, "0")}</strong><span>Hours</span></div><div><strong>{String(parts.minutes).padStart(2, "0")}</strong><span>Minutes</span></div><div><strong>{String(parts.seconds).padStart(2, "0")}</strong><span>Seconds</span></div><div><strong>{new Date(orb.startsAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</strong><span>Launch</span></div></div>}
