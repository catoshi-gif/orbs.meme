...
export default function OrbLobbyHero({ orb, winner = false }: { orb: PublicOrbRecord; winner?: boolean }) {
  ...
  return <section className="card prize-hero real-orb-hero">
    ...
    {closed ? <>
      <div className="live-launch-panel closed"><span>{winner ? "FIRST FINISH VERIFIED" : "COMPETITION ENDED"}</span><strong>{winner ? "The winner lock is secured and the result is ready." : "The six-hour race window closed without a verified winner."}</strong><a className="btn-primary" href={`/orb/${orb.slug}/results`}>View result →</a></div>
    ...
    </> : <WaitingRoomPresence slug={orb.slug} details={live ? <>
      <div className="live-launch-panel"><span>THE ORB IS LIVE</span><strong>Same maze. Same start. First verified finish wins.</strong><a className="btn-primary" href="#qualify">Register to enter →</a></div>
      <div className="commitment-card"><span>SEALED GAME COMMITMENT</span><code>{orb.commitment}</code><small>No maze seed, geometry, path or checkpoints are returned before launch.</small></div>
    </> : <>
