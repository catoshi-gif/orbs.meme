...
export default function OrbLobbyHero({ orb, winner = false }: { orb: PublicOrbRecord; winner?: boolean }) {
  ...
  return <section className="card prize-hero real-orb-hero">
    ...
    {closed ? <>
      <div className="live-launch-panel closed"><span>{winner ? (gameType === "arena" ? "ARENA WINNER VERIFIED" : "FIRST FINISH VERIFIED") : "COMPETITION ENDED"}</span><strong>{winner ? "The winner lock is secured and the result is ready." : "The competition window closed without a verified winner."}</strong><a className="btn-primary" href={`/orb/${orb.slug}/results`}>View result →</a></div>
