...
export default function OrbLobbyHero({ orb, winner = false }: { orb: PublicOrbRecord; winner?: boolean }) {
  ...
  const difficulty = gameType === "arena" ? "ARENA · live multiplayer" : gameType === "race" ? "RACE · 3 laps · live multiplayer" : orb.difficulty === "quick" ? "MAZE · Easy · ~2 minute target" : orb.difficulty === "classic" ? "MAZE · Medium · ~4 minute target" : "MAZE · Hard · ~6 minute target";
  ...
  return <section className="card prize-hero real-orb-hero">
    ...
    <div className="orb-top"><div className="hostline">{orb.hostX.profileImageUrl ? <img className="host-avatar" src={orb.hostX.profileImageUrl} alt="" referrerPolicy="no-referrer" /> : <span className="avatar" />}Hosted by @{orb.hostX.username}<span className="game-type-chip">{gameType.toUpperCase()}</span></div><span className={`pill ${live && !closed ? "live" : ""}`}>{closed ? (winner ? "WINNER VERIFIED" : "EXPIRED") : live ? "LIVE" : "SEALED"}</span></div>
    ...
    {closed ? <>
      <div className="live-launch-panel closed"><span>{winner ? (gameType === "arena" ? "ARENA WINNER VERIFIED" : gameType === "race" ? "RACE WINNER VERIFIED" : "FIRST FINISH VERIFIED") : "COMPETITION ENDED"}</span><strong>{winner ? "The winner lock is secured and the result is ready." : "The competition window closed without a verified winner."}</strong><a className="btn-primary" href={`/orb/${orb.slug}/results`}>View result →</a></div>
      <div className="commitment-card"><span>SEALED GAME COMMITMENT</span><code>{orb.commitment}</code><small>{gameType === "maze" ? "No maze seed, geometry, path or checkpoints are returned before launch." : gameType === "race" ? "The procedural Race course remains sealed until the authoritative start." : "Arena configuration is committed before launch."}</small></div>
    </> : <WaitingRoomPresence slug={orb.slug} gameType={gameType} details={live ? <>
      <div className="live-launch-panel"><span>THE ORB IS LIVE</span><strong>{gameType === "arena" ? "ARENA is live. Enter with your registered Orb." : gameType === "race" ? "RACE is live. Three laps. First across the final finish wins." : "Same maze. Same start. First verified finish wins."}</strong><a className="btn-primary" href="#qualify">Register to enter →</a></div>
      <div className="commitment-card"><span>SEALED GAME COMMITMENT</span><code>{orb.commitment}</code><small>{gameType === "maze" ? "No maze seed, geometry, path or checkpoints are returned before launch." : gameType === "race" ? "The procedural Race course remains sealed until the authoritative start." : "Arena configuration is committed before launch."}</small></div>
    </> : <>
