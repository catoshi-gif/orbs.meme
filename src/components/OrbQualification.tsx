...
export default function OrbQualification({ slug, hostXId, hostUsername, createdAt, startsAt, endsAt, prizeTokenAmount, prizeUsd, tokenSymbol }: Props) {
  ...
  return <aside id="qualify" className="card qualify">
    ...
    <div className="q-list">
      ...
      <div className={`q-row ${eligibilityConfirmed ? "ready" : ""}`}><span className="q-num">{eligibilityConfirmed ? "✓" : "4"}</span><div className="q-copy"><strong>18+ eligibility</strong><small>{eligibilityConfirmed ? "Eligibility receipt saved for this wallet" : walletVerified ? "Neutral age check · date of birth is not stored" : "Verify your wallet first"}</small></div>{walletVerified ? <EligibilityGate compact onChange={setEligibilityConfirmed} /> : null}</div>
      <div className={`q-row q-human ${humanVerified ? "ready" : ""}`}><span className="q-num">{humanVerified ? "✓" : "5"}</span><div className="q-copy"><strong>Human check</strong><small>{humanVerified ? "Cloudflare challenge verified" : "Bot resistance before a competitive session is issued"}</small></div><TurnstileGate key={`${slug}:${xUser?.id || "none"}:${wallet}`} slug={slug} wallet={wallet} enabled={readyForHuman} onVerified={setHumanVerified} /></div>
