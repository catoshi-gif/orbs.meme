...
export default function AdminArenaSandbox(){
  ...
  return <section className="admin-game-sandbox admin-arena-sandbox">
    <div className="admin-sandbox-heading">
      <div><span className="eyebrow">Multiplayer laboratory</span><h2>Arena game generator</h2><p>Local-only Last Orb Standing power coliseum with computer opponents. Tune population, world palette, pacing and capture format here while testing a focused four-ring power system, recovery rings, pedestal jumps and vertical combat before Arena is exposed to creators.</p></div>
    ...
    <div className="admin-sandbox-builder card">
      <div className="admin-sandbox-section">
        ...
        <div className="arena-builder-population">
          ...
          <div className="admin-sandbox-difficulties arena-pace-select">
            <button type="button" className={draft.pace==="demo"?"active":""} onClick={()=>update("pace","demo")}><strong>Demo pace</strong><small>~105 sec · video/tuning</small></button>
            <button type="button" className={draft.pace==="production"?"active":""} onClick={()=>update("pace","production")}><strong>10 min cap</strong><small>production pacing model</small></button>
          </div>
        </div>
      ...
      <div className="admin-sandbox-section">
        ...
        <p className="arena-builder-note">The creator controls the world palette and their own Orb. The coliseum uses broad courts, ramps, hills, bounce objects, solid terrain ramps, jumpable power pedestals and a multi-level central Orb dais; every opponent receives a distinct generated Orb color.</p>
    ...
    {active?<div className="admin-sandbox-stage-shell"><div className="admin-sandbox-stage-toolbar"><div><strong>ARENA · LAST ORB STANDING</strong><span>{active.playerCount} Orbs</span><span>{arenaRadiusForPlayers(active.playerCount).toFixed(1)}m footprint</span><span>{active.pace==="demo"?"demo pace":"10-minute cap"}</span></div><button type="button" className="btn-secondary" onClick={()=>void fullscreen()}>Fullscreen capture ↗</button></div><div ref={captureRef} className={`admin-sandbox-capture arena-capture ${format}`}><ArenaSandbox key={`${active.generation}:${format}`} playerCount={active.playerCount} style={{marble:active.marble,marbleSecondary:active.marbleSecondary,walls:active.walls,floor:active.floor,accent:active.accent}} seed={active.seed} pace={active.pace} generation={active.generation}/></div></div>:<div className="admin-sandbox-empty card arena-empty"><div className="admin-sandbox-empty-orb"/><strong>The Arena is offline.</strong><span>Choose a population, palette and seed above, then enter the mountain power coliseum with computer opponents, jumpable pedestals, double jump, blaster, super speed, health recovery and the central Orb monument.</span></div>}
  </section>;
}
