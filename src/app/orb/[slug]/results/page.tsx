...
export default async function Page({params}:{params:Promise<{slug:string}>}) {
  ...
  return <div className="page"><div className="container"><div className="result">
    ...
    <span className="eyebrow">{winner ? "Orb cleared" : "Orb result"}</span>
    <h1>{winner ? "The first light was found." : "The race is still open."}</h1>
    <p className="muted">{winner ? <>{winner.xUsername ? <><strong>@{winner.xUsername}</strong> · </> : null}{winner.wallet ? <><code>{winner.wallet.slice(0,6)}…{winner.wallet.slice(-6)}</code> · </> : null}server-verified deterministic finish.</> : <>Winner state for <strong>{slug}</strong>. The prize remains reserved until there is one server-verified first finish.</>}</p>
    <div className="result-prize gradient-text">{prize}</div>
    <button className="btn-primary" disabled>{winner ? "Claim prize · Turnkey + Anchor integration next" : "Waiting for a verified winner"}</button>
