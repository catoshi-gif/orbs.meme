...
async function loadRemoteImage(value:string|null){if(!value)return null;try{const r=await fetch(value,{cache:"force-cache",signal:AbortSignal.timeout(3000)}),type=(r.headers.get("content-type")||"").split(";")[0].toLowerCase();if(!r.ok||!["image/jpeg","image/png","image/webp"].includes(type))return null;const bytes=Buffer.from(await r.arrayBuffer());if(!bytes.length||bytes.length>2_000_000)return null;return `data:${type};base64,${bytes.toString("base64")}`;}catch{return null;}}
