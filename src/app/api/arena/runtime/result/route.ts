...
export async function POST(request: Request) {
  ...
  if (body.schemaVersion!==1 || body.version!==ARENA_GAME_VERSION || !body.matchId || !body.orbId || !body.slug) return NextResponse.json({ok:false,error:"Invalid Arena result"},{status:400});
  const orb=await getOrbRecord(body.slug);
  if (!orb || orb.id!==body.orbId || orbGameType(orb)!=="arena") return NextResponse.json({ok:false,error:"Arena Orb mismatch"},{status:404});
