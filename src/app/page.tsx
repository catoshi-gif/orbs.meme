...
export default async function Home() {
  ...
  return <>
    <section className="hero hero-game-backdrop"><OrbShowcase background className="home-orb-showcase"/><div className="hero-grid"><div><div className="eyebrow">MAZE · ARENA · LIVE ON SOLANA</div><h1>Where online communities <span className="gradient-text">play for their tokens.</span></h1><p>Host a game, put up a token prize, and bring your community, or jump into a live Orb and play for someone else&apos;s.</p><div className="hero-actions"><a className="btn-primary" href="#live">Find a live Orb</a><Link className="btn-secondary" href="/create">Create an Orb</Link></div><div className="trust"><span>Free to enter</span><span>Skill decides</span><span>Prize escrowed before launch</span></div></div><div className="visual"><Image src="/orbs-logo-512.png" width={512} height={512} alt="" priority/><div className="visual-card"><span className="eyebrow">{next ? "Starting soon" : orbs.some((orb) => orb.startsAt <= now) ? "Game live" : "Next Orb"}</span><strong>{next ? formatLaunch(next.startsAt, now) : orbs.some((orb) => orb.startsAt <= now) ? "LIVE" : "—"}</strong><small>{orbs.length ? "MAZE and ARENA. One link, one launch, one winner." : "No public Orbs are waiting right now."}</small></div></div></div></section>


    <section className="section section-soft" id="live"><div className="container"><div className="section-head"><div><span className="eyebrow">Play now</span><h2>Live & upcoming Orbs.</h2></div><p>Pick your game. Join the community. Play for the prize.</p></div>
