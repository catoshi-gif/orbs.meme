...
export default async function Home() {
  ...
  const featured = next ?? orbs.find((orb) => orb.startsAt <= now);

  return <>
    <section className="hero"><div className="hero-grid"><div><div className="eyebrow">The gamified viral marketing protocol</div><h1>Put up a reward. <span className="gradient-text">Drop an Orb.</span> Grow your community.</h1><p>Create a live glass-marble challenge for your followers. Fund the prize with a Solana token. First valid finish wins.</p><div className="hero-actions"><Link className="btn-primary" href="/create">Create an Orb</Link><a className="btn-secondary" href="#live">Find a live Orb</a></div><div className="trust"><span>Free to play</span><span>Skill decides</span><span>Prize escrowed before launch</span></div></div><div className="visual"><div className="home-cinematic-shell"><OrbCinematic style={featured?.style}/><div className="cinematic-glass-label"><span className="cinematic-live-dot"/>A glimpse of the race</div></div><div className="visual-card"><span className="eyebrow">{next ? "Starting soon" : orbs.some((orb) => orb.startsAt <= now) ? "Race live" : "Next Orb"}</span><strong>{next ? formatLaunch(next.startsAt, now) : orbs.some((orb) => orb.startsAt <= now) ? "LIVE" : "—"}</strong><small>{orbs.length ? "One link. One maze. First valid finish wins." : "No funded public Orbs are waiting right now."}</small></div></div></div></section>
