...
export default async function Home() {
  ...
  return <>
    <section className="hero hero-game-backdrop"><OrbShowcase background className="home-orb-showcase"/><div className="hero-grid"><div><div className="eyebrow">The gamified viral marketing protocol</div><h1>Put up a reward. <span className="gradient-text">Drop an Orb.</span> Grow your community.</h1><p>Create a MAZE or ARENA for your followers. Fund the prize with a Solana token and turn launch into a live community event.</p><div className="hero-actions"><Link className="btn-primary" href="/create">Create an Orb</Link><a className="btn-secondary" href="#live">Find a live Orb</a></div><div className="trust"><span>Free to play</span><span>Skill decides</span><span>Prize escrowed before launch</span></div></div><div className="visual"><Image src="/orbs-logo-512.png" width={512} height={512} alt="" priority/><div className="visual-card"><span className="eyebrow">{next ? "Starting soon" : orbs.some((orb) => orb.startsAt <= now) ? "Race live" : "Next Orb"}</span><strong>{next ? formatLaunch(next.startsAt, now) : orbs.some((orb) => orb.startsAt <= now) ? "LIVE" : "—"}</strong><small>{orbs.length ? "MAZE and ARENA. One link, one launch, one winner." : "No funded public Orbs are waiting right now."}</small></div></div></div></section>


    <section className="section section-soft" id="live"><div className="container"><div className="section-head"><div><span className="eyebrow">Live network</span><h2>Funded Orbs.</h2></div><p>Real prizes are already escrowed. Pick a MAZE or ARENA, sign up before launch, and prove your skills.</p></div>
      {orbs.length ? <div className="cards3 live-orb-grid">{orbs.map((orb) => {
        ...
        return <Link className="orb-card live-orb-card" href={`/orb/${orb.slug}`} key={orb.id}>
          ...
          <div className="live-orb-cta"><span>{live ? "Enter the live Orb" : "View Orb & sign up"}</span><strong aria-hidden="true">→</strong></div>
        </Link>;
    ...
    <section className="section"><div className="container"><div className="section-head"><div><span className="eyebrow">The loop</span><h2>Play. Win. Grow.</h2></div><p>The prize buys attention, the game turns attention into an event, and every player becomes a potential future host.</p></div><div className="steps3"><div className="card"><div className="step">1</div><h3>Fund an Orb</h3><p>Choose SOL or a supported standard SPL token, set your total commitment, pick MAZE or ARENA, customize the look, and schedule launch.</p></div><div className="card"><div className="step">2</div><h3>Share the countdown</h3><p>Your unique Orb URL becomes the social event. Players qualify, the post has time to spread, and everyone waits for the same launch.</p></div><div className="card"><div className="step">3</div><h3>One game. One winner</h3><p>When the timer hits zero, the committed game opens. The authoritative game service validates the result and the winner claims the prize.</p></div></div></div></section>
