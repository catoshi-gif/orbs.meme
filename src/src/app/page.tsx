...
export default async function Home() {
  ...
  return <>
    ...
    <section className="section section-soft" id="live"><div className="container"><div className="section-head"><div><span className="eyebrow">Live network</span><h2>Funded Orbs.</h2></div><p>Real prizes are already escrowed. Pick an Orb, sign up before launch, and prove your skills. First verified finish wins.</p></div>
      {orbs.length ? <div className="cards3 live-orb-grid">{orbs.map((orb) => {
        ...
        return <Link className="orb-card live-orb-card" href={`/orb/${orb.slug}`} key={orb.id}>
          <div className="orb-top"><div className="hostline">{orb.hostX.profileImageUrl ? <img className="host-avatar" src={orb.hostX.profileImageUrl} alt="" referrerPolicy="no-referrer"/> : <span className="avatar"/>}<span>@{orb.hostX.username}</span></div><span className={`pill ${live ? "live" : ""}`}>{live ? "LIVE NOW" : `Starts in ${formatLaunch(orb.startsAt, now)}`}</span></div>
          ...
          <div className="orb-meta"><span>{difficultyLabel(orb.difficulty)}</span><span>{qualified ? `${qualified} ${qualified === 1 ? "player" : "players"} ready` : "Open for entries"}</span></div>
